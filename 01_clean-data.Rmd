...
crs = read_csv('data/crs_data_1995_2017.csv')
...
# cepii data
cepii = haven::read_dta('data/dist_cepii.dta')
...
# get disaster data
disaster = read_csv('data/deaths-natural-disasters-ihme.csv')
...
trade1 = read_csv('data/trade1.csv')
trade2 = read_csv('data/trade2.csv')
...
#  rename(recipient=country)
cw = read_csv('data/prio_civilwars.csv')
...
# alliances
allies=read_csv('data/alliances.csv')
allies=allies%>%
  mutate(
    dcode=countrycode(
      mem1, origin = 'cown',
...
write_csv(dt9,'data/analysis_dt0.csv')

api = read_csv('data/api_data.csv')


# how does this look in the analysis?

dt = read_csv('data/analysis_dt0.csv')
dt_small = dt %>%
...
findPCA = function(vars){
  ...
  objective = function(vars,d){
    vars = cbind(1,vars)
    pc = vars%*%d
    scor = apply(vars,2,function(x)sum((pc-x)^2))
    ssc = sum(scor)
    return(-ssc)
  }
  ...
  out = optim(
    fn = objective,
    vars = vars,
    par=rep(0,len=1+ncol(vars))
  )
  
  # get the pc...
  pc = cbind(1,as.matrix(vars))%*%out$par
...
dt_na = dt_na %>%
  mutate(
    ln_aid = log(1+aid),
    ln_peer_aid = log(1+peer_aid),
    ln_peer_aid_all = log(1+peer_aid_all),
    ln_gdp = log(donor_gdp),
    ln_pop = log(donor_pop),
    ln_usmil = log(1+usmil)
  ) %>%
  group_by(donor,year) %>%
  mutate(tot_aid = sum(aid) - aid) %>%
  mutate(donor_inc = donor_gdp - tot_aid) %>%
  mutate(ln_inc = log(donor_inc)) %>%
  group_by(year) %>%
  mutate(
    ln_peer_gdp = log(sum(donor_gdp)-donor_gdp),
    ln_peer_pop = log(1/(sum(donor_pop)-donor_pop))
  ) 
...
dt_na = dt_na %>%
  ...
  mutate(
    ln_peer_aid_wt = log(1+peer_aid_wt)
  )

# save dataset for analysis
write_csv(dt_na,'data/final_data.csv')
```
